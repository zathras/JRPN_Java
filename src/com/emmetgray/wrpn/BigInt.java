...
// This is the arbitrary precision Big Integer library.  It is somewhat
// analogous to the BigInteger library except here you can set the resultant
// precision (rather than have it automatically expand).  It also contains
// a few calculator-related functions that do not appear in BigInteger
public final class BigInt {
    ...
    public BigInt(String s, int size, ArithMode mode) {
        ...
        if (s.length() > 2 && s.startsWith("&")) {
                ...
                case 'b':      // Binary (not a really a standard notation!)
                    ...
                    for (i = number.length() - 1; i >= finish; i--) {
                        ...
                        if (number.substring(i, i + 1).equals("1")) {
                            this.n[n_byte] = (byte) (this.n[n_byte] + Math.pow(2, n_bit));
                        }
                    }
                    break;
            ...
            finish = 0;
            // From right to left.  This might seem a bit counterintutive
            // since "123.45" would yield "45" and not "123"
            for (i = s.length() - 1; i >= 0; i--) {
                // Stop at first unknown character (including a decimal
                // or negative sign)
                c = s.substring(i, i + 1);
    ...
    public void Add(BigInt y, int size) {
        ...
        real_bitsize = (size > 0) ? size : ((BitSize >= temp_y.BitSize) ? BitSize : temp_y.BitSize);
    ...
    public void Multiply(BigInt y, int size) {
        ...
        real_bitsize = (size > 0) ? size : ((temp_x.BitSize >= temp_y.BitSize) ? temp_x.BitSize : temp_y.BitSize);
    ...
    public void Power(BigInt y, int size) {
        ...
        if (IsZero() || (temp_y.SignBit() && (pBIArithMode != ArithMode.Unsigned))) {
            //  zero or negative
            SetSize(real_bitsize);
            Clear();
            return;
    ...
    public void BitwiseAnd(BigInt y, int size) {
        ...
        SetSize((size > 0) ? size : ((BitSize >= temp_y.BitSize) ? BitSize : temp_y.BitSize));
    ...
    public static BigInt RotateRight(BigInt x, int distance, boolean WithCarry, boolean StartingCarry, int size) {
        BigInt ans = x.Copy();
    ...
    public static BigInt RotateLeft(BigInt x, int distance, boolean WithCarry, boolean StartingCarry, int size) {
        BigInt ans = x.Copy();
    ...
    public String ToStringBin(boolean pad) {
        ...
        if (pad) {
            ans = sb.toString().substring(sb.length() - (BitSize + spaces), sb.length());
        } else {
    ...
    private void SetSize(int size) {
        if (size != BitSize) {
            BigInt temp = null;
            if ((size > BitSize) && (SignBit() && (pBIArithMode != ArithMode.Unsigned))) {
    ...
    // Put the integer value at the specified starting bit location within
    // the byte array
    ...
    private int GetAt(int Location, int Length) {
        ...
        if (n_byte < LastByte) {
            temp = (int) (n[n_byte] & 0xff) + (int) (n[n_byte + 1] & 0xff) * 256;
        } else {
    ...
    private static int GetInt(byte[] b) {
        java.io.ByteArrayInputStream bais = new java.io.ByteArrayInputStream(reverse(b));
