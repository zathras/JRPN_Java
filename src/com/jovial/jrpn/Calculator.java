...
public class Calculator {
    ...
    // Digits (0-9 and A-F) use their own values, all others use a grid system
    // like this:
    ...
    public enum k {
        ...
        FnSL(k.KeyA.index() + F_KEY), FnSR(k.KeyB.index() + F_KEY), FnRL(
        k.KeyC.index() + F_KEY), FnRR(k.KeyD.index() + F_KEY), FnRLn(
        k.KeyE.index() + F_KEY), FnRRn(k.KeyF.index() + F_KEY), FnMASKL(
        ...
        k.KeyCHS.index() + G_KEY), FnXeq0(k.KeyAdd.index() + G_KEY), Refresh(
        -1);
                ...
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        temp.setBiVal(BigInt.Divide(cs.getStack().Pop().getBiVal(),
                                y));
                        cs.setFlag(CalcState.CalcFlag.Carry, temp.getBiVal().isCarryBit());
                        cs.setFlag(CalcState.CalcFlag.Overflow, temp.getBiVal().isOverflow());
                    }
            ...
            case KeyMul: // Multiplication
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        temp.setBiVal(BigInt.Multiply(cs.getStack().Pop().getBiVal(), cs.getStack().Pop().getBiVal()));
                        cs.setFlag(CalcState.CalcFlag.Overflow, temp.getBiVal().isOverflow());
                    }
                    Packet.setMenuNeedsUpdating(true);
                    cs.getStack().Push(temp);
                ...
                if (PrgmEntry) {
                    ...
                    if (cs.getPrgmMemory().size() > 0) {
                        ...
                        Packet.setAlternateText(String.format("%1$03d",
                                ...
                                + "- "
                                + cs.getPrgmMemory().get(cs.getPrgmPosition()).substring(0, 8));
                        Packet.setDelay(Integer.parseInt(fmMain.prop.getProperty("SleepDelay")) / 2);
            ...
            case KeyMin: // Subtraction
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        temp.setBiVal(BigInt.Subtract(cs.getStack().Pop().getBiVal(), y));
                        cs.setFlag(CalcState.CalcFlag.Carry, temp.getBiVal().isCarryBit());
                        cs.setFlag(CalcState.CalcFlag.Overflow, temp.getBiVal().isOverflow());
                    }
                    Packet.setMenuNeedsUpdating(true);
...
//		return Packet;
            ...
            case KeySTO: // Store a value to a register
                ...
                val = Stage.pop();
                if (val >= Integer.parseInt(fmMain.prop.getProperty("NumRegisters"))
                ...
                if (PrgmEntry) {
                    ...
                    if (val == k.FnI.index()) {
                        ...
                        if (i >= Integer.parseInt(fmMain.prop.getProperty("NumRegisters"))) {
                            Packet.setAlternateText("Error 3 - Improper Register Number");
                            return Packet;
                        }
            ...
            case KeyCHS: // Change sign
                if (PrgmEntry) {
                    ...
                    if (RawDisplay.length() > 0) {
                        ...
                        if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                            cs.getStack().getX().setFVal(-cs.getStack().getX().getFVal());
                        } else {
                            // LastX is updated only in the integer mode (page 100)
                            cs.setRegLastX(cs.getStack().getX().Copy());
                            cs.getStack().getX().getBiVal().ChangeSign();
                            cs.setFlag(CalcState.CalcFlag.Overflow, cs.getStack().getX().getBiVal().isOverflow());
                            Packet.setMenuNeedsUpdating(true);
                        }
                        StackDisable = false;
                    }
            ...
            case KeyAdd: // Addition
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        temp.setBiVal(BigInt.Add(cs.getStack().Pop().getBiVal(), cs.getStack().Pop().getBiVal()));
                        cs.setFlag(CalcState.CalcFlag.Carry, temp.getBiVal().isCarryBit());
                        cs.setFlag(CalcState.CalcFlag.Overflow, temp.getBiVal().isOverflow());
                    }
                    Packet.setMenuNeedsUpdating(true);
            ...
            case FnSL: // Shift left
                ...
                if (PrgmEntry) {
                    ...
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnSR: // Shift Right
                ...
                if (PrgmEntry) {
                    ...
                    cs.setRegLastX(cs.getStack().getX().Copy());
                    StackDisable = false;
                    cs.getStack().getX().getBiVal().RightShift(1, false, cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRL: // Rotate Left
                ...
                if (PrgmEntry) {
                    ...
                    cs.setRegLastX(cs.getStack().getX().Copy());
                    StackDisable = false;
                    cs.getStack().getX().getBiVal().RotateLeft(1, false, false, cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRR: // Rotate Right
                ...
                if (PrgmEntry) {
                    ...
                    cs.setRegLastX(cs.getStack().getX().Copy());
                    StackDisable = false;
                    cs.getStack().getX().getBiVal().RotateRight(1, false, false, cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRLn: // Rotate Left n times
                ...
                if (PrgmEntry) {
                    ...
                    cs.getStack().getX().getBiVal().RotateLeft(val, false, false, cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRRn: // Rotate Right n times
                ...
                if (PrgmEntry) {
                    ...
                    cs.getStack().getX().getBiVal().RotateRight(val, false, false, cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            ...
            case FnShowHex: // Show HEX
                ...
                if (PrgmEntry) {
                    ...
                    Packet.setAlternateText(FormatDisplay(CalcState.CalcOpMode.Hex,
                            0));
                    Packet.setDelay(Integer.parseInt(fmMain.prop.getProperty("SleepDelay")));
                    return Packet;
                }
                break;
            ...
            case FnClearReg: // Clear all storage registers
                if (PrgmEntry) {
                    ...
                    for (int i = 0; i <= Integer.parseInt(fmMain.prop.getProperty("NumRegisters")) - 1; i++) {
                        cs.setReg(i,
                                new Register(cs.getWordSize(), cs.getArithMode()));
                    }
            ...
            case FnNOT: // The Bitwise NOT operator
                ...
                if (PrgmEntry) {
                    ...
                    cs.setRegLastX(cs.getStack().getX().Copy());
                    StackDisable = false;
                    cs.getStack().getX().getBiVal().OnesComplement(cs.getWordSize());
                }
                break;
            ...
            case FnFLOAT: // set the float mode
                ...
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() != CalcState.CalcOpMode.Float) {
                        ...
                        cs.setRegLastX(new Register(cs.getWordSize(), cs.getArithMode()));
                        cs.SyncValues();
            ...
            case FnMEM: // Display the Memory configuration
                ...
                Packet.setAlternateText(String.format("P-%1$03d R-%2$03d", Integer.parseInt(fmMain.prop.getProperty("PrgmMemoryLines")),
            ...
            case FnSTATUS: // Display the Calculator configuration
                ...
                int FlagVal = 0;
                for (int i = 0; i <= 3; i++) {
                    FlagVal += cs.isFlag(CalcState.CalcFlag.toCalcFlag(i)) ? (int) (Math.pow(10, i)) : 0;
                }
            ...
            case FnASR: // Arithmetic Shift
                ...
                if (PrgmEntry) {
                    ...
                    cs.setRegLastX(cs.getStack().getX().Copy());
                    StackDisable = false;
                    cs.getStack().getX().getBiVal().RightShift(1, true, cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRLC: // Rotate Left with Carry
                ...
                if (PrgmEntry) {
                    ...
                    cs.setRegLastX(cs.getStack().getX().Copy());
                    StackDisable = false;
                    cs.getStack().getX().getBiVal().RotateLeft(1, true, cs.isFlag(CalcState.CalcFlag.Carry), cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRRC: // Rotate Right with Carry
                ...
                if (PrgmEntry) {
                    ...
                    cs.setRegLastX(cs.getStack().getX().Copy());
                    StackDisable = false;
                    cs.getStack().getX().getBiVal().RotateRight(1, true, cs.isFlag(CalcState.CalcFlag.Carry), cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRLCn: // Rotate Left with Carry n times
                ...
                if (PrgmEntry) {
                    ...
                    cs.getStack().getX().getBiVal().RotateLeft(val, true, cs.isFlag(CalcState.CalcFlag.Carry), cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            case FnRRCn: // Rotate Right with Carry n times
                ...
                if (PrgmEntry) {
                    ...
                    cs.getStack().getX().getBiVal().RotateRight(val, true, cs.isFlag(CalcState.CalcFlag.Carry), cs.getWordSize());
                    cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                    Packet.setMenuNeedsUpdating(true);
                }
                break;
            ...
            case FnABS: // Absolute value
                if (PrgmEntry) {
                    ...
                    StackDisable = false;
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        cs.getStack().getX().setFVal(Math.abs(cs.getStack().getX().getFVal()));
                    } else {
            ...
            case FnDSZ: // Decrement, skip on zero (Program Mode)
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        cs.getRegIndex().getBiVal().Subtract(BigInt.One(cs.getWordSize()),
                                cs.getWordSize());
            ...
            case FnISZ: // Increment, skip on zero (Program Mode)
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        // v6.0.8 - thanks Andru
                        cs.getRegIndex().getBiVal().Add(BigInt.One(cs.getWordSize()), cs.getWordSize());
                        if (cs.getRegIndex().getBiVal().IsZero()) {
                            cs.setPrgmPosition(cs.getPrgmPosition() + 1);
                        }
                    }
            ...
            case FnSqrt: // Square root
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        cs.getStack().getX().setFVal(Math.sqrt(cs.getStack().getX().getFVal()));
                    } else {
                        BigInt zero = new BigInt(cs.getWordSize(),
                                BigInt.ArithMode.toArithMode(cs.getArithMode().index()));
                        if (cs.getStack().getX().getBiVal().IsLessOrEqual(zero)) {
                            Packet.setAlternateText("Error 0 - Improper Math Operation");
                            Prefix = 0;
                            return Packet;
                        }
                        cs.getStack().getX().getBiVal().SquareRoot(cs.getWordSize());
                        cs.setFlag(CalcState.CalcFlag.Carry, cs.getStack().getX().getBiVal().isCarryBit());
                        Packet.setMenuNeedsUpdating(true);
                    }
                }
            ...
            case FnInv: // Inverse
                ...
                if (PrgmEntry) {
                    ...
                    cs.getStack().getX().setFVal(1.0 / cs.getStack().getX().getFVal());
                }
                break;
            case FnSF: // Set Flag
                // This is a multi-stage command, so we have to store 1
            ...
            case FnXlteY: // Is X less than or equal to Y (Program Mode)
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        if (cs.getStack().getX().getFVal() > cs.getStack().getY().getFVal()) {
                            cs.setPrgmPosition(cs.getPrgmPosition() + 1);
                        }
                    } else {
                        if (cs.getStack().getX().getBiVal().IsGreater(cs.getStack().getY().getBiVal())) {
                            cs.setPrgmPosition(cs.getPrgmPosition() + 1);
                        }
                    }
                }
            ...
            case FnXgtY: // Is X greater than Y (Program Mode)
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        if (cs.getStack().getX().getBiVal().IsLessOrEqual(cs.getStack().getY().getBiVal())) {
                            cs.setPrgmPosition(cs.getPrgmPosition() + 1);
                        }
                    }
                }
            ...
            case FnSRight: // Scroll the display Right
                ...
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Bin
                            && cs.getWordSize() > 32) {
                        int len = cs.getStack().getX().getBiVal().ToStringBin(true).length();
                        WinPos++;
                        if (WinPos > len - 35) {
            ...
            case FnXneY: // Is X not equal to Y (Program Mode)
                if (PrgmEntry) {
                    ...
                    if (cs.getOpMode() == CalcState.CalcOpMode.Float) {
                        ...
                        if (cs.getStack().getX().getBiVal().Equals(cs.getStack().getY().getBiVal())) {
                            cs.setPrgmPosition(cs.getPrgmPosition() + 1);
                        }
                    }
                }
        ...
        if (PrgmEntry) {
            ...
            if (cs.getPrgmMemory().size() == Integer.parseInt(fmMain.prop.getProperty("PrgmMemoryLines"))) {
                Packet.setAlternateText("Error 4 - Improper Line Number");
                cs.getPrgmMemory().remove(
                        Integer.parseInt(fmMain.prop.getProperty("PrgmMemoryLines")));
                cs.setPrgmPosition(Integer.parseInt(fmMain.prop.getProperty("PrgmMemoryLines")) - 1);
                return Packet;
            }
            ...
            if (cs.getPrgmPosition() > 0) {
                Packet.setDisplayText(String.format("%1$03d",
                        cs.getPrgmPosition())
                        + "- "
                        + cs.getPrgmMemory().get(cs.getPrgmPosition() - 1).substring(0, 8));
            } else {
    ...
    private void ConvertInput(String text) {
        ...
        BigInt.ArithMode bimode = BigInt.ArithMode.toArithMode(cs.getArithMode().index());
        switch (cs.getOpMode()) {
            case Float:
        ...
        switch (Op) {
            case Float:
                ...
                if (cs.getFloatPrecision() == k.KeyDp.index()) {
                    ...
                    // the real calculator, so I just picked 15... I hope you like
                    // it
                    Formatter = "%1$.15e";
                } else {
                    Formatter = "%1$." + cs.getFloatPrecision() + "f";
                }
                DisplayText = String.format(Formatter, cs.getStack().getX().getFVal());
                ...
                if (DisplayText.length() > 37
                        ...
                        + StringStrDup('0', cs.getFloatPrecision())) && cs.getStack().getX().getFVal() != 0.0)) {
                    DisplayText = String.format("%1$.15e", cs.getStack().getX().getFVal());
                }
                break;
            case Hex:
